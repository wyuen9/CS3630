...
def particle_likelihood(robot_marker_list: list[MarkerMeasure], particle_marker_list: list[MarkerMeasure]) -> float:
    ...
    l = 1.0
    ######### START STUDENT CODE #########
    # Step 1: Generate marker pairs, and store unmatched markers.
    matched_markers, unmatched_robot_markers, unmatched_particle_markers = generate_marker_pairs(robot_marker_list, particle_marker_list)

    # Step 2: Compute the likelihood using matched markers.
    for robot_marker, particle_marker in matched_markers:
        marker_prob = marker_likelihood(robot_marker, particle_marker)
        l *= marker_prob

    # Step 3: incorporate the likelihood of unmatched markers.
    for marker in unmatched_robot_markers:
        l *= compute_spurious_detection_rate(marker)
    for marker in unmatched_particle_markers:
        l *= compute_detection_failure_rate(marker)
...
def comptue_particle_weights(particles:list[SE2], robot_marker_measures:list[MarkerMeasure], env:Environment) -> list[float]:
    ...
    for particle in particles:
        ...
        likelihood = particle_likelihood(robot_marker_measures, particle_marker_measures)
