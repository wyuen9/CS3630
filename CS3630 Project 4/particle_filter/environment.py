...
class Environment:
    ...
    def read_marker_measures(self, T_w_r: SE2) -> list[MarkerMeasure]:
        ...
        Hints:
            ...
            * You may find the following naming conventions helpful:
                ...
                - P_a to represent a point expressedin the coordinate frame a.
        ...
        T_r_w = T_w_r.inverse()
        T_c_r = self.T_r_c.inverse()
        T_c_w = T_c_r.compose(T_r_w)

        # Step 2: Compute the lidar coordinate frame
        T_l_r = self.T_r_l.inverse()
        T_l_w = T_l_r.compose(T_r_w)
        # Iterate over all markers
        for Pm_w in self.markers:

            # Step 3: Compute the marker expressed in the camera frame
            Pm_c = T_c_w.transform_point(Pm_w)

            # Step 4: Compute the depth and angle (in radians)
            depth = Pm_c.x
            angle = math.atan2(Pm_c.y, depth)

            # Step 5: Compute the marker expressed in the lidar frame
            Pm_l = T_l_w.transform_point(Pm_w)

            # Step 6: Compute the distance of marker in the lidar frame
            lidar_range = math.sqrt(Pm_l.x**2 + Pm_l.y**2)

            # Step 7: Check if the marker if visible to the camera, append to marker_measures if visible.
            if self.visible(T_w_r, Pm_w):
                marker_measures.append(MarkerMeasure(depth, angle, lidar_range))
    ...
    def diff_drive_kinematics(self, omega_l: float, omega_r: float) -> tuple[float, float]:
        ...
        ######### START STUDENT CODE #########
        v_x = (omega_l + omega_r) / 2 * self.wheel_radius
        omega = (omega_r - omega_l) / self.axle_length * self.wheel_radius
